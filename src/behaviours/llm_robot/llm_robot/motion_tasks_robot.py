...
class MotionRobot(Node):
    ...
    def vlm_pick_and_place(self, **kwargs):#识别、抓取、放置
        try:
            ...
            # 从嵌套的字典中获取具体的参数
            self.pick_objects = nested_kwargs.get('object_1', '未指定')
            self.place_object = nested_kwargs.get('object_2', '未指定')

            # 如果 kwargs 不是嵌套的，则直接获取参数
            if not nested_kwargs:
                self.pick_objects = kwargs.get('object_1', '未指定')
                self.place_object = kwargs.get('object_2', '未指定')
            if self.pick_objects and self.place_object:
                self.bb.set("pick_objects",self.pick_objects)
                self.bb.set("place_object",self.place_object)
